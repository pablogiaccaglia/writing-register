...
- `notion`, `mail`, `discord`: before Claude creates or updates a page with the Notion connector (`notion-create-pages`, `notion-update-page`), sends an email with Apple Mail (`compose_email`, `reply_to_email`) or posts to Discord (`discord_send_message`, `discord_send_dm`), the plugin rewrites the prose Claude added and the call goes out with the new text. For a page update, only the text that is new against what it replaces is rewritten. Right after the call, Claude is shown each passage that changed, old and new. On ten Notion payloads a new page took half a minute to a minute and an update 12 to 21 seconds. If a rewrite is refused or takes longer than 90 seconds, the text goes out as Claude wrote it; an email with an HTML body is never rewritten.
