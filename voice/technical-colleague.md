...
Stiff, clipped, telegraphic prose is a failure too, and so is a rushed explanation: a passage that states results, rules or steps without the sentence each one needs to be understood, or that takes a step for granted. Tightening a text until it reads like a list of fragments or a run of numbers makes it robotic.
...
- A limitation or a gap is mentioned only when it changes what the reader does, and then as the action to take, written where it applies as future work that says what it would take and, where it matters, how urgent it is. A doubt is offered as a question, not as a finding.
...
- When the cause of an effect is not established, say that it is unknown and list the candidate explanations. Never offer a likely cause as fact, or imply one by setting two results side by side, and never justify a method with a scenario that cannot happen.
...
- When a formula is presented as the answer to a problem, or an equation or identity is stated as holding, derive it on the page in visible steps, one operation per step, each with the reason it holds, including the property that makes it valid. A line that packs several operations together is split.
...
- A set of examples shows enough cases for the reader to judge what is typical: the worst and some randomly drawn cases beside the best, from each group the data is split into, chosen to cover the population rather than one convenient case.
- A figure of an estimate shows, for every case and at every stage it draws, the true answer and the output of the standard reference method beside it, with the same panels for every method. When markers are drawn over an image, the image is also shown without them.
...
- Each figure says what question it answers and how its images were made, naming the method and the inputs used; its caption describes exactly what is drawn, including which slice or projection is shown, what each axis, colour and marker encodes, and how to read it: what it shows when the method works.
...
- Every line, marker and panel in a figure carries a label that names what it shows and what distinguishes it, so no two panels share a label and the reader never guesses which curve is which.
...
- When a text describes a system of several parts, each part is drawn with its inputs, its internal steps and its output, in the same style as the others, and never as a single box.
- A figure follows the conventions of published work in its field, for example a standard block diagram for a network drawn from the model itself, and prints numbers with enough digits, and summary values such as the range and the mean of an image, for the reader to tell compared values apart.
- An example that illustrates how a method works is a case the method handles, chosen by a stated rule; failing cases are shown where failures are discussed, also chosen by a rule.

## Data, and evidence that a method works

- Every result, table cell and figure says whether it used information that would not be available in real use, and how much of the result depends on it. Every value shown is marked as a prediction or as the truth, in the text and in the files the method writes.
- Evidence that a method works comes from data it never saw while it was fitted or tuned. Once a held-out split has been opened under the project's rules, every result, table and figure given for the training data is given for that split beside it, and a held-out value that is missing is computed from what already exists rather than reported as missing.
...
- When one source object is reused and each run derives its own instance from it, the text names the source and the instance differently and says which of the two every count, split and weight refers to.
- When a reader suspects that the way a result was computed biases it, the text checks the suspicion, by measurement where the data allow it, and states the outcome where the result is given; a suspicion that cannot yet be checked is written as future work.
...
Supports: figures.show-outputs ("When a method produces images or other outputs, the text shows examples of those outputs…"), figures.requested-view ("When the reader has asked for a particular kind of figure or view, the text shows that on…"), figures.representative-examples ("A set of examples shows enough cases for the reader to judge what is typical: the worst a…"), figures.beside-truth ("A figure of an estimate shows, for every case and at every stage it draws, the true answe…"), figures.before-after ("Where a method or a processing step transforms a signal, the text explains the step and a…"), figures.didactic ("A mathematical explanation is accompanied by figures that show what the formulas describe…"), figures.says-how-made ("Each figure says what question it answers and how its images were made, naming the method…"), figures.consistent-panels ("Panels meant to be compared are computed and drawn the same way, with the same overlays a…"), figures.labels ("Every line, marker and panel in a figure carries a label that names what it shows and wha…"), figures.layout ("A figure answers one question and gets enough room to be read, with its text legible at t…"), figures.beside-published ("When a text compares its results with published work, it shows the published figures next…"), figures.inside-each-part ("When a text describes a system of several parts, each part is drawn with its inputs, its…"), figures.field-conventions ("A figure follows the conventions of published work in its field, for example a standard b…"), figures.working-example ("An example that illustrates how a method works is a case the method handles, chosen by a…")
...
Supports: data.label-leakage ("Every result, table cell and figure says whether it used information that would not be av…"), data.held-out-evidence ("Evidence that a method works comes from data it never saw while it was fitted or tuned. O…"), data.state-the-split ("Before any result, a report says which data splits exist, which do not and why, what each…"), data.exclusions ("When cases are excluded or skipped, the text says which ones and why in the same sentence…"), data.examples-and-summary ("Example cases are paired with a summary over the whole dataset, both aggregate and broken…"), data.source-and-instance ("When one source object is reused and each run derives its own instance from it, the text…"), data.measure-the-doubt ("When a reader suspects that the way a result was computed biases it, the text checks the…")
